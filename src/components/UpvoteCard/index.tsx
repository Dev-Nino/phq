...
const UpvoteCard = ({ props }: { props: Feedback }) => {
  ...
  const hasFollowed =
    ...
    followIdeaLogs
      ...
      .map((log) => log.user_id ?? log.anonymous_id)
      .includes(user.id);
  ...
  return (
    ...
    >
      ...
      <div className="upvote-details w-full">
        <UpvoteLabelLink id={props.id?.toString()} className="idea-h w-full">
          {props.title}
          {isAuthenticated() && (
            <button
              className={`text-sm text-[#6b7280] border border-[#6b7280] rounded-md flex items-center justify-center ${
                hasFollowed
                  ? 'bg-purple-50 text-purple-700 px-2'
                  : 'hover:bg-[#f3f4f6] hover:text-[#09041a] p-2'
              } gap-1.5`}
              data-tooltip-content={`${hasFollowed ? 'Unfollow' : 'Follow'}`}
              onClick={() => {
                if (props.id) {
                  if (hasFollowed) {
                    unfollowIdea(props.id);
                    return;
                  }
                  followIdea(props.id);
                }
            ...
            >
              <EyeIcon />
              {followIdeaLogs.length > 0 ? followIdeaLogs.length : ''}
            </button>
          )}
        </UpvoteLabelLink>
