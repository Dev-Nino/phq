...
type FeedbackAction =
  ...
  | { type: 'SET_FOLLOW_IDEA_LOGS'; payload: FollowIdeaLog[] }
...
interface FeedbackContextType {
  ...
  followIdea: (id: number) => Promise<void>;
  ...
  listFollowIdeaLogs: (id: number) => Promise<void>;
  ...
  setFollowIdeaLogs: (logs: FollowIdeaLog[]) => Promise<void>;
  ...
  unfollowIdea: (id: number) => Promise<void>;
...
function feedbackReducer(
  ...
  switch (action.type) {
    ...
    case 'SET_FOLLOW_IDEA_LOGS':
      return { ...state, followIdeaLogs: action.payload };
    case 'SET_IDEAS':
...
export function FeedbackProvider({ children }: { children: ReactNode }) {
  ...
  const followIdea = async (id: number) => {
    postApi<FollowIdeaLog>({ url: `feedback/${id}/follow` }).then((res) => {
      if (res.results.data) {
        setFollowIdeaLogs([...followIdeaLogs, res.results.data]);
      }
    });
  };
  ...
  const listFollowIdeaLogs = async (id: number) => {
    getApi<FollowIdeaLog[]>({ url: `feedback/${id}/follow/logs` }).then(
      (res) => {
        if (res.results.data) {
          setFollowIdeaLogs(res.results.data);
        }
      }
    );
  };
  ...
  const setFollowIdeaLogs = async (logs: FollowIdeaLog[]) => {
    dispatch({ type: 'SET_FOLLOW_IDEA_LOGS', payload: logs });
  };
  ...
  const unfollowIdea = async (id: number) => {
    deleteApi<FollowIdeaLog>({ url: `feedback/${id}/follow` }).then((res) => {
      if (res.results.data) {
        setFollowIdeaLogs(
          followIdeaLogs.filter((log) => log.id !== res.results.data?.id)
        );
      }
    });
  };
